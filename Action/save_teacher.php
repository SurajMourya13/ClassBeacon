...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        // --- Teacher Profile Data (for TEACHERS table) ---
        ...
        // 2. Create the teacher profile (Includes ALL new fields)
        $sql_teacher = "INSERT INTO teachers (user_id, full_name, gender, dob, starting_year, nationality, profile_picture, email, phone, created_at) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
        // Bind parameters: i (user_id), s (full_name), s (gender), s (dob), s (startYear), s (nationality), s (profile_picture), s (email), s (phone)
        $stmt_teacher = $conn->prepare($sql_teacher);
        $stmt_teacher->bind_param("issssssss", 
            $user_id, 
            $fullName, 
            $gender, 
            $dob, 
        ...
        $conn->rollback();
        // Clean up the user record if the teacher profile insert failed
        if (isset($user_id)) {
        ...
        if (isset($stmt_teacher) && $stmt_teacher) $stmt_teacher->close();
        if ($conn) $conn->close();
    }
